...
export default function DashboardPage() {
  ...
  // Laisse une unité d'air au-dessus de la plus haute barre pour que le badge
  // (étoile + compteur) affiché au sommet ne soit jamais rogné.
  const practitionerYMax = useMemo(() => {
    let max = 0;
    for (const row of monthlyPractitionerData) {
      for (const name of practitionerNames) {
        const value = Number(row[name]) || 0;
        if (value > max) max = value;
      }
    }
    return max + 1;
  }, [monthlyPractitionerData, practitionerNames]);
  ...
  if (!mounted || isLoading || !isAuthenticated || isRedirecting) {
        ...
        <CardContent>
          <ResponsiveContainer width="100%" height={300}>
            ...
            >
              ...
              <YAxis allowDecimals={false} domain={[0, practitionerYMax]} tick={{ fontSize: 12 }} />
              <RechartsTooltip
                contentStyle={{ borderRadius: 8, border: '1px solid #e5e7eb' }}
